...
export default function PageD({ setCurrentPage, currentPage }: componentProp) {
    const dispatch = useDispatch();
    const { updateUser } = bindActionCreators(actionCreators, dispatch);

    const { user } = useSelector((state: State) => state);

    const [bvn, setBvn] = useState<string>(user.bvn);
                ...
                >
                    ...
                    <div className=" flex flex-col my-10 space-y-4">
                        ...
                        <div className="input relative flex w-full">
                            ...
                            />
                            <img
                                className=" w-5 absolute top-[17px] right-[30px]"
                                src={CHECK}
                                alt=""
                            />
                        </div>
